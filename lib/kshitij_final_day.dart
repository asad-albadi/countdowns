...
class KshitijFinalDayCountdownPage extends StatefulWidget {
  const KshitijFinalDayCountdownPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _KshitijFinalDayCountdownPageState createState() =>
      _KshitijFinalDayCountdownPageState();
}

class _KshitijFinalDayCountdownPageState
    extends State<KshitijFinalDayCountdownPage> {
  late DateTime now;
  ...
  void initState() {
    ...
    now = DateTime.now();
    finalDate =
        DateTime(2024, 9, 25, 15, 0, 0); // Wedding on October 18, 2024 at 6 PM
    countdownDuration = finalDate.difference(now);
  ...
  Future<void> _playSound() async {
    try {
      final url = Uri.base.resolve('assets/sound/goodbye.mp3').toString();
          ...
          Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                Text(
                  'Final Day Date: ${format.format(finalDate)}',
                  style:
                      TextStyle(color: Colors.white, fontSize: fontSize + 10),
                ),
              ],
